...
# 格式1: ext4-sdcard 裸镜像
EXT4_IMG=$(ls bin/targets/sunxi/cortexa7/*nanopi-r1s-h3*ext4-sdcard.img.gz 2>/dev/null | head -1)
if [ -f "$EXT4_IMG" ]; then
    gunzip -c "$EXT4_IMG" > "$OUTPUT_DIR/immortalwrt-sunxi-cortexa7-friendlyarm_nanopi-r1s-h3-ext4-sdcard-${DATE_ONLY}.img"
    echo "✅ ext4-sdcard 镜像"
fi

# 格式2: squashfs-sdcard 裸镜像
SQUASHFS_IMG=$(ls bin/targets/sunxi/cortexa7/*nanopi-r1s-h3*squashfs-sdcard.img.gz 2>/dev/null | head -1)
if [ -f "$SQUASHFS_IMG" ]; then
    gunzip -c "$SQUASHFS_IMG" > "$OUTPUT_DIR/immortalwrt-sunxi-cortexa7-friendlyarm_nanopi-r1s-h3-squashfs-sdcard-${DATE_ONLY}.img"
    echo "✅ squashfs-sdcard 镜像"
fi

# 格式3: rootfs.tar
ROOTFS_TAR=$(ls bin/targets/sunxi/cortexa7/*nanopi-r1s-h3*rootfs.tar.gz 2>/dev/null | head -1)
